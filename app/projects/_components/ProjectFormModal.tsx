...
export default function ProjectFormModal({
  ...
  const handleTestConnection = async () => {
    const uriResult = projectFormSchema.pick({ mongodbUri: true }).safeParse({ mongodbUri });
          ...
          <div className="p-6 space-y-4">
            <div className="flex flex-col space-y-1.5">
              ...
              <input
                ...
                className={`w-full px-4 py-2 bg-canvas border rounded-sm text-sm text-ink outline-hidden transition-all h-11 focus:ring-1 focus:ring-ink focus:border-ink ${
                  nameError ? 'border-primary-error-text' : 'border-hairline'
                }`}
              />
              {nameError && (
                <p className="text-xs text-primary-error-text font-medium mt-1">{nameError}</p>
              )}
            </div>
            ...
            <div className="flex flex-col space-y-1.5">
              <label className="text-xs font-semibold text-ink uppercase tracking-wider">
                Description{' '}
                <span className="text-muted text-[10px] font-normal lowercase">(optional)</span>
              </label>
              <textarea
                value={description}
            ...
            <div className="flex flex-col space-y-1.5">
              ...
              <Select
                value={category}
                onValueChange={(val) =>
                  setCategory(val as 'production' | 'staging' | 'development')
                }
              >
            ...
            <div className="flex flex-col space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`text-xs font-semibold px-2 py-0.5 rounded-xs transition-all cursor-pointer ${
                    testingConnection
              ...
              <input
                ...
                className={`w-full px-4 py-2 bg-canvas border rounded-sm font-mono text-xs text-ink outline-hidden transition-all h-11 focus:ring-1 focus:ring-ink focus:border-ink ${
                  uriError ? 'border-primary-error-text' : 'border-hairline'
                }`}
              />
              ...
              {connectionStatus === 'error' && (
                <p className="text-xs text-primary-error-text font-medium flex items-center gap-xs mt-1 animate-fade-in">
                  <span className="font-bold">⚠</span> Connection failed: Invalid cluster domain or
                  auth credentials.
                </p>
              )}
              {uriError && (
                <p className="text-xs text-primary-error-text font-medium mt-1">{uriError}</p>
              )}

              <p className="text-[11px] text-body">
                Supports connection strings starting with{' '}
                <code className="font-mono bg-surface-soft px-1 py-0.5 rounded">mongodb://</code> or{' '}
