...
export default function ProjectCard({
  ...
  return (
    <div
      className={`relative rounded-md border bg-canvas p-base flex flex-col justify-between transition-all duration-200 cursor-pointer shadow-sm hover:shadow-airbnb hover:border-hairline-soft ${isSelected
    ...
    >
      <div className="flex justify-between items-start mb-md">
        ...
        >
          <input
            ...
            className={`h-4.5 w-4.5 rounded border-hairline text-primary focus:ring-primary accent-primary ${project.isArchived ? "cursor-not-allowed opacity-50" : "cursor-pointer"
              }`}
      ...
      <div className="flex-1 min-w-0 mb-md" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-xs mb-xs flex-wrap">
          <h3 className="text-title-md font-semibold text-ink truncate flex-1">
            {project.name}
          </h3>

          {project.category && (
            <span
              className={`text-[9px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full border shrink-0 ${project.category === "production"
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <span className={`font-mono text-xs text-ink truncate flex-1 leading-tight ${revealUri ? "select-all" : "select-none cursor-default"}`}>
          {revealUri ? project.mongodbUri : getCensoredUri(project.mongodbUri)}
        </span>
        <div className="flex items-center gap-xs shrink-0">
          <span
            className={`h-2.5 w-2.5 rounded-full mr-xs transition-all duration-300 ${project.connectionStatus === "connected"
          ...
          <Button
            variant="ghost"
            size="icon"
            className={`h-7 w-7 rounded-full text-body hover:text-ink hover:bg-canvas transition-all ${isTesting ? "animate-spin text-primary" : ""
              }`}
      ...
      <div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <span className="text-uppercase-tag text-muted font-medium">
        ...
        <div className="flex items-center gap-xs">
          <Button
            variant="ghost"
            size="icon"
            className={`h-8 w-8 rounded-full text-body hover:bg-surface-soft ${project.isArchived
